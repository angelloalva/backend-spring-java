...
public class DoctorController {
    ...
    // Obtener turnos de un doctor (todos los roles pueden ver)
    @GetMapping("/{id}/turnos")
    public ResponseEntity<List<Turno>> obtenerTurnosDeDoctor(@PathVariable String id) {
        return doctorService.obtenerDoctorPorId(id)
                .map(doctor -> {
                    // Aquí deberías llamar al TurnoService para obtener los turnos actualizados
                    List<Turno> turnos = turnoService.obtenerTurnosPorDoctor(id);
